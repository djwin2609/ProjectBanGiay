...
public class ProductDao {
	public List<ProductDetailBean> GetProduct() throws SQLException {
	...
	public List<ProductDetailBean> SearchProducts(String keyword) throws SQLException {
		List<ProductDetailBean> ds = new ArrayList<>();
		KetNoiCSDL kn = new KetNoiCSDL();
		kn.connectcsdl();
		String sql =  "SELECT  p.Product_id,p.ProductName,p.Price, p.Image,pd.ProductDetail_id,pd.Size,pd.Brand,pd.Stock_quantity,pd.Status,pd.Description FROM Product p join ProductDetail pd ON p.Product_id = pd.Product_id where ProductName like ?";
		PreparedStatement psSearch = kn.cn.prepareStatement(sql);
		psSearch.setString(1, "%" + keyword + "%");
